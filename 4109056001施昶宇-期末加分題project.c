...
int check(char name[]){
		...
		while(current){//if name==opcode return 1
			if(name[0]=='+'||!strcmp(name,"BASE")||!strcmp(current->name,name)||!strcmp(name,"START")||!strcmp(name,"RSUB")||!strcmp(name,"BYTE")||!strcmp(name,"WORD")||!strcmp(name,"RESB")||!strcmp(name,"RESW")||!strcmp(name,"END"))
				return 1;
			current=current->link;
		}
	return 0;
...
void loc(){
	...
	while(current->link!=NULL){
		...
		if(!strcmp(current->opcode,"RESB")){//opcode==RESB
			...
			dectohex(hextodec(num)+3,hex);
			strcpy(current->location,hex);
			dectohex((hextodec(current->location)+1),hex);
			strcpy(num,hex);//1039
		}
		else if(!strcmp(current->opcode,"CLEAR")||!strcmp(current->opcode,"TIXR")||!strcmp(current->opcode,"COMPR")){//opcode==BYTE
			dectohex(hextodec(num)+3,hex);
			strcpy(current->location,hex);
			dectohex((hextodec(current->location)-1),hex);
			strcpy(num,hex);//1039
